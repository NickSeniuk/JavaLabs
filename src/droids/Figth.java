...
public class Figth {
    ...
    public static void startFightOnetoOne(AlphaDroid attacker, AlphaDroid defender) // Атакуючий дроїд атакує дроїда, який захищається
    {
    ...
    public static void startFightTeamRound(Team t1, Team t2){
        ...
        AlphaDroid RedAttack, RedDef;
        AlphaDroid BlueAttack, BlueDef;
        while(true)
        {
            ...
            if(RedAttack != null && RedAttack.type == "Protector" && ((Protector)RedAttack).getAura() > 0)
            {
                AlphaDroid pacient = t1.GetDroidReadyToAura(RedAttack);
                if (pacient!= null) {
                    ((Protector) RedAttack).setAura(pacient);
            ...
            if(BlueAttack != null && BlueAttack.type == "Protector" && ((Protector)BlueAttack).getAura() > 0)
            {
                AlphaDroid pacient = t2.GetDroidReadyToAura(BlueAttack);
                if (pacient!= null) {
                    ((Protector) BlueAttack).setAura(pacient);
